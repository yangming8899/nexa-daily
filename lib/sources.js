...
/** 429 自动退避重试:第一次失败后等 20s 再试,再失败等 40s */
async function fetchWithBackoff(url, opts = {}, maxRetries = 2) {
  for (let i = 0; i <= maxRetries; i++) {
    try {
      ...
      const waitSec = 20 * (i + 1);
      console.log(`[sources] 429 限流,${waitSec}s 后重试 #${i + 1}…`);
