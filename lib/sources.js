...
/** 解析 nitter HTML 抽出推文列表 */
function parseNitterHtml(html, host, username) {
  const tweets = [];
  // 每条推文包裹在 <div class="timeline-item ...">...</div>
  // 用一个简单状态机找块:从 <a class="tweet-link" href="..."> 开始,到下一个 timeline-item 结束
  const re = /<a class="tweet-link"\s+href="([^"]+)"[^>]*><\/a>[\s\S]*?<div class="tweet-content[^"]*"[^>]*>([\s\S]*?)<\/div>[\s\S]*?<span class="tweet-date"[^>]*>(?:<a[^>]+title="([^"]+)"[^>]*>[^<]+<\/a>|<\/span>)/g;
  let m;
  while ((m = re.exec(html)) && tweets.length < PER_USER_LIMIT) {
    const href = m[1];              // /username/status/123#m
    const contentHtml = m[2];
    const fullDate = m[3] || '';    // "Jun 30, 2026 · 3:00 PM UTC"
    // 清理 HTML 标签和多余空白
...
async function fetchAllUpdates(people) {
  ...
  // 并行抓取,每个用户串行 syndication→nitter,整体快很多
