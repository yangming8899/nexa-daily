...
const TIMEOUT_MS = 12_000; // 12s (arxiv RSS 504KB 经常慢)
const PROXY_TOKEN = process.env.SCRAPE_DO_TOKEN;

/** 通过代理或直连抓取文本,429 时自动回退到直连 */
async function fetchTextResilient(url, { headers = {}, method = 'GET', body = null } = {}) {
  // 优先代理
  if (PROXY_TOKEN) {
    try {
      ...
      console.warn(`[sources] 代理 ${e.message.slice(0, 60)},回退直连 ${url}`);
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        return fetchTextResilient(res.headers.location, { headers }).then(resolve, reject);
      }
...
async function fetchWithBackoff(url, opts = {}) {
  return await fetchTextResilient(url, opts);
}
...
function formatArxivDate(s) {
  // arxiv 格式: "Thu, 09 Jul 2026 00:00:00 -0400"
...
async function fetchAllUpdates(people) {
  ...
  // 并行抓取(限制并发 3),单用户超时由 fetchText 内部 6s 控制
  // Vercel 免费版函数 10s 限制,这里加一个全局 9s 总闸
  const GLOBAL_BUDGET_MS = 9_000;
  const start = Date.now();
  const results = await Promise.all(tasks.map(async ({ p, username }) => {
    const remain = () => Math.max(0, GLOBAL_BUDGET_MS - (Date.now() - start));
    try {
      console.log(`  → ${username} (剩 ${Math.ceil(remain()/1000)}s)`);
      const tweets = remain() > 1500
        ? await fetchUserTweets(username)
        : [];
      return { ok: true, tweets, p, username };
    } catch (e) {
      console.warn(`  ✗ ${username}: ${e.message}`);
      return { ok: false, err: e.message, tweets: [], p, username };
    }
  }));
...
module.exports = {
  fetchText: fetchTextResilient, parseRssItems, decodeEntities,
