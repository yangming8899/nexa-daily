...
function stripThinking(content) {
  if (!content) return '';
  const m = content.match(/<\/think\s*>/i);
  if (m) return content.slice(m.index + m[0].length).trim();
  // 备选: 第一个 ## 或 ### 之前是思考
  const h = content.match(/(^|\n)(#{1,3})\s/);
  if (h) return content.slice(h.index + h[0].length).trim();
  return content.trim();
}
